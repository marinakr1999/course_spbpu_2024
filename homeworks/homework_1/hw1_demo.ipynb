...
def calc_pipe(
    ...
    result = solve_ivp(
        ...
        method='RK23'               # Метод Рунге-Кутта 3 порядка
    )
...
if __name__ == "__main__":
    ...
    with open(base_dir.parent.joinpath("input_data", "11.json"), "r") as input_file:
        input_data = json.load(input_file)

    # Создаем диапазон дебитов жидкости для генерации VLP
    q_liq_list = list(range(0, 400, 10))

    # Рассчитываем забойные давления для каждого значения дебита жидкости (VLP)
    p_wf_list = [calc_p_wf(
        ...
        q_ms=q_liq_temp / 86400,
        d_tub=input_data["d_tub"],
        roughness=input_data["roughness"]
    ) / 101325 for q_liq_temp in q_liq_list]
    ...
    with open(base_dir.joinpath("output.json"), "w") as output_file:
